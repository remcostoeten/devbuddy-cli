  ...
  for (const file of files) {
    ...
    if (imports.length > 0) {
      ...
      if (dryRun) {
        ...
        logger.info(chalk.gray('Original imports:'))
        imports.forEach((imp) => logger.info(chalk.gray(`  ${imp.originalImport}`)))
        logger.info(chalk.gray('Consolidated import:'))
        console.log(
          chalk.gray(
        ...
        logger.info(chalk.green(`Updated imports in ${file}`))
      }
    }
  }
