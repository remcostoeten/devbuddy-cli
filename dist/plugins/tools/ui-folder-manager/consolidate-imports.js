 ...
 */
export async function consolidateImports(uiFolder, dryRun) {
    ...
    for (const file of files) {
        ...
        if (imports.length > 0) {
            ...
            if (dryRun) {
                ...
                console.log(chalk.gray(`  import { ${imports.map((imp) => imp.componentName).join(", ")} } from '${getRelativePath(file, uiFolder)}';`));
            }
            else {
...
function extractImports(content, uiFolder) {
...
function replaceImports(content, imports, uiFolder) {
...
function getRelativePath(filePath, uiFolder) {
